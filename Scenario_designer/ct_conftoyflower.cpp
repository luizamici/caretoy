...
#include "ui_ct_conftoyflower.h"

#define NUM_LIGHTS 3
#define NUM_SPEAKERS 1

CTConfToyFlower::CTConfToyFlower(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CTConfToyFlower)
{
    ui->setupUi(this);
    ...
    ui->qcb_force->addItems(QStringList() << "all" << "left" << "middle" << "right" );
}


void CTConfToyFlower::on_qrb_null_event_toggled(bool checked)
{
    ui->tabFeedbackAction->setDisabled(checked);
}

CTConfToyFlower::~CTConfToyFlower()
{
    delete ui;
}
...
bool CTConfToyFlower::setParameters(QDomElement root)
{
    ...
    if ("block" != root.tagName() || "flower" != root.attribute("name"))
    {
        return false;
    }
    ...
    else if ("force" == event_name)
    {
        ...
        ui->qcb_force->setCurrentIndex(ui->qcb_force->findText(sensor));
        double value =
    ...
    for (int i = 0; i < num_actions; i++)
    {
        ...
        if (i < NUM_LIGHTS)
        {
            ...
            /*
             *The min e max duration are set into one spin box for all the actions
             */
 ...
 */
QString CTConfToyFlower::getParameters(QString value){
...
//    stream.writeStartDocument();
    ...
    else if (ui->qrb_force_event->isChecked())
    {
        ...
        stream.writeAttribute("name", "force");
...
QDomElement CTConfToyFlower::getParameters()
{
    ...
    else if (ui->qrb_force_event->isChecked())
    {
        ...
        event.setAttribute("sensor",ui->qcb_force->currentText());
        QDomElement condition = doc.createElement("condition");
        event.appendChild(condition);
 ...
 */
double CTConfToyFlower::calculateRequiredTime()
{
...
/*!
 * \brief CTConfToyFlower::updateBlockRuntime
 *
 ...
 */
void CTConfToyFlower::updateBlockRuntime(bool value)
{
    value = false;
 ...
 * \param value This parameter is unused.
 */
void CTConfToyFlower::updateBlockRuntime(double value)
{
    value = 0;
    ui->qsb_block_duration->setValue(calculateRequiredTime());
