...
#include "ui_ct_conftoyu.h"

#define NUM_LIGHTS 4
#define NUM_SPEAKERS 1

CTConfToyU::CTConfToyU(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CTConfToyU)
{
    ui->setupUi(this);
    ...
    connect(ui->qrb_pressure_event,SIGNAL(toggled(bool)),
            ui->qcb_pressure,SLOT(setEnabled(bool)));
    ...
    ui->qcb_pressure->addItems(QStringList() << "all" << "left" << "right" );
}

CTConfToyU::~CTConfToyU()
{
    delete ui;
}
...
bool CTConfToyU::setParameters(QString xml)
{
     ...
     */
    handler->setWidget(5, this, num_stimuli, num_actions);
    QList<CTLight*> empty;
 ...
 */
bool CTConfToyU::setParameters(QDomElement root)
{
    /* Check if the supplied configuration is applicable */
    if ("block" != root.tagName() || "utoy" != root.attribute("name"))
    {
        return false;
    }
...
/*!
 * \brief CTConfToyU::getParameters
 *
 ...
 */
QString CTConfToyU::getParameters(QString value){
    ...
    stream.writeAttribute("name", "utoy");
 ...
 */
QDomElement CTConfToyU::getParameters()
{
    QDomDocument doc;
    QDomElement root = doc.createElement("block");
    root.setAttribute("id", 1);
    root.setAttribute("name", "utoy");
    ...
    else if (ui->qrb_pressure_event->isChecked())
    {
        ...
        event.setAttribute("sensor",ui->qcb_pressure->currentText());
        QDomElement condition = doc.createElement("condition");
        event.appendChild(condition);
...
/*!
 * \brief CTConfToyU::calculateRequiredTime
 *
 ...
 */
double CTConfToyU::calculateRequiredTime()
{
...
/*!
 * \brief CTConfToyU::updateBlockRuntime
 *
 ...
 */
void CTConfToyU::updateBlockRuntime(bool value)
{
    value = false;
 ...
 * \param value This parameter is unused.
 */
void CTConfToyU::updateBlockRuntime(double value)
{
    value = 0;
    ui->qsb_block_duration->setValue(calculateRequiredTime());
...
void CTConfToyU::on_qrb_null_event_toggled(bool checked)
{
