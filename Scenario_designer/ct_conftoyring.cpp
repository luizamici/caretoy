...
#include "ui_ct_conftoyring.h"

#define NUM_LIGHTS 2
#define NUM_SPEAKERS 1

CTConfToyRing::CTConfToyRing(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CTConfToyRing)
{
    ui->setupUi(this);
...
void CTConfToyRing::on_qrb_null_event_toggled(bool checked)
{
    ui->tabFeedbackAction->setDisabled(checked);
}

CTConfToyRing::~CTConfToyRing()
{
    delete ui;
}
 ...
 */
QString CTConfToyRing::getParameters(QString value){
 ...
 */
QDomElement CTConfToyRing::getParameters()
{
    QDomDocument doc;
    QDomElement root = doc.createElement("block");
    root.setAttribute("id", 1);
    root.setAttribute("name", "ring");
...
/*!
 * \brief CTConfToyRing::calculateRequiredTime
 *
 ...
 */
double CTConfToyRing::calculateRequiredTime()
{
...
/*!
 * \brief CTConfToyRing::updateBlockRuntime
 *
 ...
 */
void CTConfToyRing::updateBlockRuntime(bool value)
{
    value = false;
 ...
 * \param value This parameter is unused.
 */
void CTConfToyRing::updateBlockRuntime(double value)
{
    value = 0;
    ui->qsb_block_duration->setValue(calculateRequiredTime());
